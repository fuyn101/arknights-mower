...
def ocr_worker(args):
    i, data, template_images = args
    result = DepotSolver.match_once(data[i], template_images)
    return result
...
class DepotSolver(BaseSolver):
    ...
    def transition(self) -> bool:
        ...
        if self.scene() == Scene.INDEX:
            ...
            self.swipe_only((1800, 450), (-1800, 0), 100, 1)
            ...
            file_path = str(get_path("@app/screenshot/depot/new.png"))

            cv2.imwrite(file_path, newscreenshot)
            ...
            logger.info(
                f"仓库扫描结果 ：{len(matchs_dict)}{matchs_dict} 用时{time.time() - main_start_time}"
            )
        else:
    ...
    def ocr_one_page(self, screenshot_gray, circle_info):
        ...
        with Pool(processes=os.cpu_count()) as pool:
            results = pool.map(
                ocr_worker,
                zip(range(len(data)), iter(data), [self.template_images] * len(data)),
            )
